...
def test_homepage_api():
    ...
    homepage = os.getenv("ROUTER_API_ATTRACTIONS")
...
def test_article_page_api():
    ...
    api_id = requests.get(os.getenv("ROUTER_API_ATTRACTIONS"), verify=False)
    api_id = api_id.json()

    random_number = random.randint(0, len(api_id['data'])-1)
    id = api_id['data'][random_number]['id']

    article_page = os.getenv("ROUTER_API_ATTRACTION") + str(id)
